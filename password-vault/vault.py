...
def decrypt_data(key, enc_data):
    ...
    return cipher.decrypt_and_verify(
        base64.b64decode(enc_data["ciphertext"]), base64.b64decode(enc_data["tag"])
    ).decode()
...
def load_vault(key):
    ...
    return json.loads(decrypt_data(key, enc_data))
...
def save_vault(key, vault_data, salt):
    ...
    with open(VAULT_FILE, "w") as f:
        json.dump({"salt": base64.b64encode(salt).decode(), "data": encrypted}, f)
